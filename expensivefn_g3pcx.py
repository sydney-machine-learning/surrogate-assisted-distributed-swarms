...
class Evolution(object):
	def __init__(self, pop_size, dimen, max_evals,  max_limits, min_limits , problem ):
		...
		self.pop = np.random.uniform(-5, 5 , size=(self.pop_size,self.dimen))
	...
	def fit_func(self, x):    #  function  (can be any other function, model or even a neural network)
		...
		if self.problem == 1: # rosenbrock
			...
			fit = -20*np.exp(-0.2*np.sqrt(0.5*(x[0]*x[0]+x[1]*x[1]))) - np.exp(0.5*(math.cos(2*math.pi*x[0]) + math.cos(2*math.pi*x[1]))) + math.e +20 
		elif self.problem ==5: #eggholder function
			fit = -(x[1]+47)*math.sin(np.sqrt(abs(x[1]+(x[0]/2)+47))) - x[0]*math.sin(np.sqrt(abs(x[0]-x[1]-47)))
		elif self.problem ==6: #easom function
			fit = -math.cos(x[0])*math.cos(x[1])*np.exp(-((x[0]-math.pi)**2 + (x[1]-math.pi)**2 ))            
...
def main():
	...
	max_evals = 700000
	pop_size =  1000
	num_varibles = 5 #Dimension
	max_limits = np.repeat(-5, num_varibles)
	min_limits = np.repeat(5, num_varibles)
	g3pcx  = Evolution(pop_size, num_varibles, max_evals,  max_limits, min_limits , problem = 3  )
